...
@Command(name = "find-vehicle-get",
        aliases = {"fvg"},
        description = {"Finds a vehicle in the registry by its ID"})
public class FindVehicleCommandGET implements Callable<Integer> {
    ...
    private URI buildRequestUri(String serviceName) throws IOException {
        ...
        byte[] exonumMessage =  vehicleId.getBytes();
        byte[] signature = cryptoFunction.signMessage(exonumMessage, keys.getPrivateKey());
        String baseSign = BaseEncoding.base16().lowerCase().encode(signature);
        ...
        var uri = String.format("%s/api/services/%s/vehicle/example2/%s/%s",
                Config.NODE_JAVA_API_HOST, escaper.escape(serviceName), escaper.escape(vehicleId), escaper.escape(baseSign));
